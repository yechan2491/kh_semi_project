...
<body>
	...
	<div class="outer">
		<div class="wrap">
			...
			<ul class="board_paging">
				<!-- 맨 처음으로 이동하는 버튼(<<) -->
				<li><a class="arrow pprev"
					href="${contextPath }/note/receive/list?page=1${ searchParam }"></a></li>

				<!-- 이전 페이지로(<) -->
				<c:choose>
					<c:when test="${ pi.page >1 }">
						<li><a class="arrow prev"
							href="${contextPath }/note/receive/list?page=${ pi.page-1 }${ searchParam }"></a></li>
					</c:when>
					<c:otherwise>
				...
				<c:forEach var="p" begin="${ pi.startPage }" end="${ pi.endPage }">
					<li><c:choose>
							...
							<c:otherwise>
								<a
									href="${ contextPath }/note/receive/list?page=${ p }${ searchParam }">${ p }</a>
							</c:otherwise>
						</c:choose></li>
				</c:forEach>
				...
				<li><c:choose>
						<c:when test="${ pi.page < pi.maxPage }">
							<a class="arrow next"
								href="${ contextPath }/note/receive/list?page=${ pi.page+1 }${ searchParam }"></a>
						</c:when>
						<c:otherwise>
				...
				<li><a class="arrow nnext"
					href="${ contextPath }/note/receive/list?page=${ pi.maxPage }${ searchParam }"></a>
			</ul>
			<div class="pagingcComment">
				<p>수신 쪽지 ${ pi.listCount }개가 검색되었습니다.</p>
			</div>

			<div class="search_area">
				<form method="get" action="${ contextPath }/note/receive/list">
					<select id="searchCondition" name="searchCondition">
						...
						<option value="writer"
							<c:if test="${ param.searchCondition =='writer' }">selected</c:if>>보낸이</option>
	...
	<script>
		function detailView(nno){
			location.href='${contextPath}/note/receive/detail?nno='+nno;
		}
	</script>

...
/**
 * Servlet implementation class MarketplaceInsertServlet
 */
@WebServlet("/marketplace/insert")
public class MarketplaceInsertServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public MarketplaceInsertServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		request.getRequestDispatcher("/WEB-INF/views/marketplace/marketplaceInsertView.jsp").forward(request, response);
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		*/
		 ...
		 * http://www.servlets.com  > com.oreilly.servlet > cos-20.08.zip 다운로드 후 압축풀고 cos.jar 파일 lib 경로 아래 추가*/
		
		/* 1. 전송 파일 용량 제한 : 10MB로 제한 */
		int maxSize=1024*1024*10;
		
		/* 2. 웹 서버 컨테이너 경로 추출 */
		String root = request.getSession().getServletContext().getRealPath("/"); // application객체(request.getSession().getServletContext())로 부터 path 값 받아온다.
		String savePath = root + "resources\\uploadFiles\\";
		
		/* HttpServletRequest => MultipartRequest 변경 */
		// 인자로 request, 저장할경로, 이파일저장할때 최대 저장할 수 있는 용량제한, utf-8이라고하는 인코딩타입, cos.jar라이브러리에 있는 DefaultFileRenamePolicy() 요소
		MultipartRequest multiRequest 
//		= new MultipartRequest(request, savePath, maxSize, "UTF-8", new DefaultFileRenamePolicy());
		= new MultipartRequest(request, savePath, maxSize, "UTF-8", new MyfileRenamePolicy());
		
		/* 위의 MultipartRequest 객체 생성과 동시에 업로드한 파일들이 서버의 저장 경로에 저장 됨 
		 * 즉, 이후의 프로세스에 문제가 있건 없건 우선 서버에 저장 되므로 추후 프로세스에서 문제가 생길 경우
		 * 업로드 된 파일을 삭제할 것 
		 * 
		...
		for(int i =0; i<fileNames.length; i++) {
			/* contentImg는 optional이므로 파일 첨부가 되지 않았을 수도 있음
		...
		int result = new MarketplaceService().insertMarketplace(board);

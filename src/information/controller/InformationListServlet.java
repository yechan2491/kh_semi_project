...
public class InformationListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Map<String, Object> map = new InformationService().selectInformationList(page, new Search(searchCategory));
		
		
		// 베스트글 4		
		//List<Board> bestBoardList = new InformationService().selectBestGalleryList();
...
//		System.out.println(boardList);
		...
		//선생님	
		request.setAttribute("bestBoardList", map.get("bestBoardList"));
//		System.out.println("bestBoardList = " + bestBoardList);

		request.getRequestDispatcher("/WEB-INF/views/information/informationListView.jsp").forward(request, response);
		//
